...
<body class="bg-gradient-to-br from-blue-50 via-white to-indigo-50 min-h-screen">
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      ...
      $result = $conn->query("SELECT p.*, COUNT(DISTINCT a.id) as total_avaliacoes, COALESCE(AVG(a.nota), 0) as media_avaliacao FROM prestadores p LEFT JOIN avaliacoes a ON p.id = a.prestador_id GROUP BY p.id ORDER BY p.criado_em DESC");
      ...
      if ($result && $result->num_rows > 0) {
          while ($row = $result->fetch_assoc()) {
              ...
              $endereco = htmlspecialchars($row['endereco'] ?? 'Localização não informada');
              $id = $row['id'];
              ...
              $estrelas = '';
              for ($i = 1; $i <= 5; $i++) {
                  $estrelas .= $i <= $media_avaliacao ? '<i class="fas fa-star text-xs"></i>' : '<i class="far fa-star text-xs"></i>';
              }
              $avaliacoes = intval($total_avaliacoes);
              
              echo '
              <div class="bg-white rounded-2xl shadow-lg overflow-hidden card-hover cursor-pointer border border-gray-100" data-prestador-id="' . $id . '" onclick="window.location.href=\'detalhes_prestador.php?id=\' + this.dataset.prestadorId">
                <div class="relative">
                  <img src="' . $foto . '" alt="Foto de ' . $nome . '" class="w-full h-48 object-cover">
                  <div class="absolute top-3 right-3">
                    <button class="w-8 h-8 bg-white/80 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white transition-colors fav-btn" 
                            data-prestador-id="' . $id . '"
                            onclick="event.stopPropagation(); alternarFavoritoHome(this)">
                ...
                <div class="p-5">
                  <div class="flex items-start justify-between mb-3">
                    ...
                    <div class="text-right">
                      <div class="flex items-center text-yellow-400 mb-1">
                        ' . $estrelas . '
                        <span class="text-gray-600 text-xs ml-1">(' . $avaliacoes . ')</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="space-y-2 mb-4">
                    ...
                    <div class="text-sm text-gray-500">
                      ...
                      <button class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-3 py-2 rounded-lg text-xs font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all duration-200" 
                              onclick="event.stopPropagation(); window.location.href=\'detalhes_prestador.php?id=' . $id . '\'">
  ...
  <script>
    ...
    function atualizarGrid(prestadores) {
      ...
      prestadoresGrid.innerHTML = prestadores.map(prestador => `
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden card-hover cursor-pointer border border-gray-100" 
             data-prestador-id="${prestador.id}"
             onclick="window.location.href='detalhes_prestador.php?id=${prestador.id}'">
          <div class="relative">
          ...
          <div class="p-5">
            ...
            <div class="flex items-center justify-between gap-2">
              ...
              <button class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-3 py-2 rounded-lg text-xs font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all duration-200" 
                      onclick="event.stopPropagation(); window.location.href='detalhes_prestador.php?id=${prestador.id}'">
                <i class="fas fa-user mr-1"></i>Ver Perfil
              </button>
            </div>
          </div>
    ...
    // Função para favoritar na home
    function alternarFavoritoHome(btn) {
      const prestadorId = btn.dataset.prestadorId;
      const icon = btn.querySelector('i');
      const isFavorito = icon.classList.contains('fa-solid') || btn.classList.contains('favorito');
      ...
      .then(data => {
        if (data.success) {
          ...
          console.error('Erro ao alterar favorito:', data.error);
        }
      })
