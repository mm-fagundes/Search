...
<body class="bg-gradient-to-br from-blue-50 to-indigo-100 min-h-screen flex items-center justify-center p-4">
  ...
  <div class="w-full max-w-lg">
    ...
    <div class="bg-white shadow-xl rounded-2xl p-8 border border-gray-100">
            ...
            <?php
              switch($_GET['erro']) {
                ...
                case 'telefone_invalido': 
                  ...
                  echo 'Telefone inválido.' . ($det ? ' (detalhes: ' . $det . ')' : ''); 
                  break;
      ...
      <form id="cadClienteForm" action="processa_cadastro_cliente.php" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p id="senhaHelpCliente" class="text-sm text-red-600 mt-1 hidden">A senha deve ter no mínimo 8 caracteres.</p>
          </div>
  ...
  <script>
    ...
    // Bloquear envio do formulário se a senha tiver menos de 8 caracteres (melhora UX antes do envio)
    const cadClienteForm = document.getElementById('cadClienteForm');
    const senhaCliente = document.getElementById('senha');
    const senhaHelpCliente = document.getElementById('senhaHelpCliente');

    if (cadClienteForm) {
      cadClienteForm.addEventListener('submit', function(e) {
        if (senhaCliente.value.length < 8) {
          e.preventDefault();
          senhaHelpCliente.classList.remove('hidden');
          senhaCliente.focus();
        }
      });
    }
